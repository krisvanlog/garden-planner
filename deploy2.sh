...
echo "Generating src/App.jsx (With Relocate Mode)..."
cat <<'EOF' > src/App.jsx
import React, { useState, useRef, useEffect, useCallback } from 'react';
import { Plus, Minus, Image, BookOpen, Trash2, MapPin, Move, List, X, Camera, Save, Edit3, Maximize2, Search, Leaf, ExternalLink, Settings, Clock, AlertTriangle, ChevronDown, ChevronUp, GripHorizontal, Check } from 'lucide-react';
...
const GardenPlanner = () => {
  ...
  const saveData = async () => { const updatedProjects = projects.map(p => p.id === currentProject?.id ? { ...currentProject, markers, globalJournalEntries } : p); setProjects(updatedProjects); try { await fetch('/api/projects', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedProjects) }); } catch (e) {} };
  ...
  const handleImageUpload = (e) => { const file = e.target.files[0]; if (file) { const reader = new FileReader(); reader.onload = async (event) => { const resizedImage = await resizeImage(event.target.result, 1920); const newProject = { id: Date.now(), name: file.name.replace(/\.[^/.]+$/, ''), image: resizedImage, markers: [], globalJournalEntries: [] }; const updatedProjects = [...projects, newProject]; setProjects(updatedProjects); loadProject(newProject); try { await fetch('/api/projects', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedProjects) }); } catch (e) {} }; reader.readAsDataURL(file); } };
  ...
  const handleStart = (e) => {
    ...
    // 1. Check if we are relocating a specific marker
    if (relocatingMarker) {
      // Check if we hit the relocating marker to start dragging it
      const dx = relocatingMarker.x - pos.x;
      const dy = relocatingMarker.y - pos.y;
      // Larger hit area for easier mobile use
      if (Math.sqrt(dx*dx + dy*dy) < 50) {
        // We are grabbing the relocating marker
        // Don't need special state, just use relocatingMarker state in move
      } else {
        // Tapping elsewhere while relocating? 
        // Maybe we want to allow panning still? Yes.
        setIsPanning(true);
        setPanStart({ x: pos.clientX - pan.x, y: pos.clientY - pan.y });
      }
    } else {
      // Normal Mode: Only Pan allowed on drag
      if (tool === 'pan') {
        setIsPanning(true);
        setPanStart({ x: pos.clientX - pan.x, y: pos.clientY - pan.y });
      }
  ...
  const handleMove = (e) => {
    ...
    if (relocatingMarker && !isPanning) {
      // We are in relocation mode and NOT panning, so we must be moving the marker
      e.preventDefault();
      const updated = { ...relocatingMarker, x: pos.x, y: pos.y };
      setRelocatingMarker(updated);
      // Update the main markers list in real-time for visual feedback
      setMarkers(markers.map(m => m.id === updated.id ? updated : m));
    } else if (isPanning) {
      e.preventDefault();
      setPan({ x: pos.clientX - panStart.x, y: pos.clientY - panStart.y });
    }
  };
  ...
  const handleCanvasClick = (e) => {
    if (!currentProject) return;
    if (relocatingMarker) return; // Don't open dialogs or add markers while relocating
    ...
    if (hitMarker && tool !== 'pan') {
      openMarkerDialog(hitMarker);
  ...
  const startRelocation = () => {
    if (selectedItem) {
      setRelocatingMarker(selectedItem);
  ...
  const finishRelocation = (save) => {
    if (!save && relocatingMarker) {
      ...
      if (original) {
        setMarkers(markers.map(m => m.id === relocatingMarker.id ? original : m));
      }
    }
    ...
    <div className="h-screen w-screen flex flex-col bg-gray-950 text-white overflow-hidden select-none" style={{ fontFamily: '"DM Sans", sans-serif' }}>
      ...
      <div className="bg-gray-900/95 backdrop-blur border-b border-gray-800 p-3 flex items-center justify-between z-20 shrink-0">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => fileInputRef.current?.click()} className="p-2 bg-gray-800 hover:bg-gray-700 rounded-xl transition-colors"><Image className="w-5 h-5 text-gray-300" /></button>
          {currentProject && (
            <>
              <button onClick={() => setShowDeleteProjectConfirm(true)} className="p-2 bg-red-950/50 hover:bg-red-900/50 rounded-xl text-red-400 border border-red-900/30 transition-colors"><Trash2 className="w-5 h-5" /></button>
            ...
            <>
              ...
              {relocatingMarker ? (
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3 bg-amber-900/90 backdrop-blur p-3 rounded-2xl shadow-2xl border border-amber-700 z-20 pointer-events-auto items-center">
                  <div className="text-amber-100 text-sm font-medium px-2">Move {relocatingMarker.label}</div>
                  <div className="h-6 w-px bg-amber-700/50 mx-1"></div>
...
echo "To restart the application with Relocation Mode, run:"
echo "cd $BASE_DIR"
